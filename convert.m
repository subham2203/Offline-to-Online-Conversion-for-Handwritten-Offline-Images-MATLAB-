...
for i = 1:length(D)
    ...
    % Pixel Value = 2 represents the pixel has been recorded
        ...
        if(Sum == 1 || Sum == 3)
            ...
            % Find the next connected pixel and assign to the xurrent pixel
            % value
            ...
            % If Sum is 2 or 0 it is an EndPoint
                ...
                % Dequeue from P_End and assign it to the current pixel 
                    ...
                    neighborhood = I(current_Y-20:current_Y+20,current_X-20:current_X+20);
%                     imtool(neighborhood,[])
%                     pause;
                    % Get the C Points
                    ...
                    if(length(C_Points(:,1)) > 1)
                        ...
                        %% now the left out elements in the junction point are inserted to the front of P_End for first priority 
                        C_Points = setdiff(C_Points, stored_Val,'rows');
                        for m = 1:length(C_Points(:,1))
                            dist = [];
                            for k = 1:length(N_End)
                                pt = C_Points(m,:);
                                pt(1) = pt(1) + current_Y - 21;
                                pt(2) = pt(2) + current_X - 21;
