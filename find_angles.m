...
if(~isempty(find(ref(:,2)==1)))
    ...
    else if(~isempty(find(ref(:,2)==width)))
            pos = find(ref(:,2)==width);
            ref_1 = ref(pos,:);
            ref_2 = ref(find(ref(:,2)==setdiff(ref(:,2),width)),:);
            ref_X = ref_2(2) - ref_1(2);
            ref_Y = ref_2(1) - ref_1(1);
        else if(~isempty(find(ref(:,1)==width)))
                pos = find(ref(:,1)==width);
                ref_1 = ref(pos,:);
                ref_2 = ref(find(ref(:,1)==setdiff(ref(:,1),width)),:);
                ref_X = ref_2(2) - ref_1(2);
                ref_Y = ref_2(1) - ref_1(1);
            end
        end
